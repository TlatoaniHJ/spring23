...
Proof.
  ...
  ht1.
  ht1.
  ht; unfold max3_spec; exists x8; exists x5; exists x2.
  - assert (Init.Nat.max x5 x2 = x2) by linear_arithmetic.
    assert (Init.Nat.max x8 x2 = x2) by linear_arithmetic.
    equality.
  - assert (Init.Nat.max x5 x2 = x5) by linear_arithmetic.
    assert (Init.Nat.max x8 x5 = x5) by linear_arithmetic.
    equality.
  - assert (Init.Nat.max x5 x2 = x2) by linear_arithmetic.
    assert (Init.Nat.max x8 x2 = x2) by linear_arithmetic.
    equality.
  - assert (Init.Nat.max x5 x2 <= x8) by linear_arithmetic.
    assert (Init.Nat.max x8 (Init.Nat.max x5 x2) = x8) by linear_arithmetic.
    equality.
Qed.
...
Definition euclidean_algorithm_invariant a b : assertion :=
   fun/inv tr _ v => Nat.gcd (v $! "a") (v $! "b") = Nat.gcd a b /\ tr = [].
...
Proof.
  ...
  ht.
  - rewrite <- H.
    Search (Nat.gcd ?x (?y - ?x) = Nat.gcd ?x ?y).
    apply Nat.gcd_sub_diag_r.
    linear_arithmetic.
  - rewrite <- H.
    Search (Nat.gcd ?x ?y = Nat.gcd ?y ?x).
    rewrite Nat.gcd_comm.
    transitivity (Nat.gcd (x $! "b") (x $! "a")).
    + apply Nat.gcd_sub_diag_r.
      linear_arithmetic.
    + apply Nat.gcd_comm.
  - exists (v $! "a").
...
Definition fibonacci_invariant (n: nat) : assertion :=
   fun/inv tr _ v => exists x y rem,
      fibonacci_spec tr /\ tr = Out y :: Out x :: rem /\ v $! "y" = y /\ v $! "x" = x.
...
Definition fact_invariant (n: nat) : assertion :=
   fun/inv tr _ v => exists rem,
      (v $! "cnt") <= (v $! "n") /\ fact_spec (v $! "cnt") tr /\ tr = Out (v $! "x") :: rem /\ v $! "n" = n.
...
Definition mailbox_invariant : assertion :=
   fun/inv tr h v => (v $! "done" = 0 /\ mailbox_spec h tr) \/ (v $! "done" = 1 /\ mailbox_done h tr).
...
Lemma array_at_left (h : heap) : forall (left right : list nat) ptr ,
  array_at h ptr (left ++ right) -> array_at h ptr left.
Proof.
  simplify.
  induct left.
  - apply ArrayEmpty.
  - rewrite <- app_comm_cons in H.
    ...
    apply ArrayCons.
    + assumption.
    + apply IHleft with (right).
      assumption.
Qed.

Lemma array_at_right (h : heap) : forall (left right : list nat) ptr ,
  array_at h ptr (left ++ right) -> array_at h (ptr + length left) right.
Proof.
  simplify.
  induct left.
  - simplify.
    assert (ptr + 0 = ptr) by linear_arithmetic.
    rewrite H0.
    assumption.
  - inversion H.
    simplify.
    assert (ptr + S (length left) = S ptr + (length left)) by linear_arithmetic.
    rewrite H5.
    apply IHleft.
    assumption.
Qed.
...
Definition search_invariant (ptr: nat) (data: list nat) : assertion :=
   fun/inv tr h v => exists prefix suffix,
      length prefix = (v $! "length") /\ data = prefix ++ suffix /\
      search_spec (v $! "needle") (v $! "length") suffix tr /\ array_at h ptr data /\ v $! "ptr" = ptr.
...
Lemma length_0 (data : list nat) : length data = 0 -> data = [].
Proof.
  cases data;
    simplify;
    equality.
...
Lemma length_not_0 (data : list nat) : length data > 0 -> data <> [].
Proof.
  cases data.
  - simplify.
    linear_arithmetic.
...
Proof.
  ...
  - rewrite <- H in *.
    ...
    + pose (exists_last H0).
      ...
      exists x3.
      exists (x4 :: x2).
      assert (Datatypes.length x3 = Datatypes.length x1 - 1).
      * rewrite e0.
        pose (app_length x3 [x4]).
        simplify.
        linear_arithmetic.
      * propositional.
        -- rewrite e0.
           rewrite app_ass.
           auto.
        -- apply SearchConsYes; auto.
           assert (array_at h ((x0 $! "ptr") + (Datatypes.length x1 - 1)) [x4]).
           ++ rewrite <- H1.
              apply array_at_right.
              rewrite <- e0.
              apply array_at_left with (right := x2).
              assumption.
           ++ invert H2.
              equality.
  - rewrite <- H in *.
    ...
    + pose (exists_last H0).
      ...
      exists x2.
      exists (x3 :: x1).
      assert (Datatypes.length x2 = Datatypes.length x0 - 1).
      * rewrite e.
        pose (app_length x2 [x3]).
        simplify.
        linear_arithmetic.
      * propositional.
        -- rewrite e.
           rewrite app_ass.
           auto.
        -- apply SearchConsNo; auto.
           assert (array_at h ((x $! "ptr") + (Datatypes.length x0 - 1)) [x3]).
           ++ rewrite <- H1.
              apply array_at_right.
              rewrite <- e.
              apply array_at_left with (right := x1).
              assumption.
           ++ invert H2.
              equality.
  - assert (x = []).
