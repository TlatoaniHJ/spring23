...
Theorem mtree'_null : forall {t r}, r = 0
  -> mtree' t r === [| t = Leaf |].
Proof.
  heq; cases t; cancel.
Qed.

Lemma mtree'_nonnull : forall {t r}, r <> 0
  -> mtree' t r === exists t1 t2 x r1 r2, [| t = Node t1 x t2 |] * r |--> [r1; x; r2] * mtree' t1 r1 * mtree' t2 r2.
Proof.
  heq; cases t; cancel.
  - equality.
  - invert H0; cancel.
...
Theorem mtree_nonnull : forall {r}, r <> 0
  -> mtree r === exists r1 x r2, r |--> [r1; x; r2] * mtree r1 * mtree r2.
Proof.
...
Theorem mtree_null : forall r, r = 0
  -> mtree r === emp.
Proof.
  unfold mtree; simplify.
...
Proof.
  ...
  step.
  simp.
  loop_inv (fun r' => (p |-> r * [|p <> 0|] * mtree r')%sep) (fun r' (_ : bool) => (p |-> r * [|p <> 0|] *  mtree r')%sep).
  - cases (acc ==n 0).
    + step.
      cancel.
    + rewrite mtree_nonnull by assumption.
      ...
      * cases (x <=? r0).
        -- step.
           step.
           step.
           apply exis_right with (x := (acc |-> r1 * (acc + 1) |-> r0 * (exists n1, (acc + 1 + 1) |-> n1 * mtree n1))%sep).
           cancel.
           rewrite (mtree_nonnull n).
           cancel.
        -- step.
           step.
           step.
           apply exis_right with (x := ((acc + 1 + 1) |-> r1 * (acc + 1) |-> r0 * (exists n1, acc |-> n1 * mtree n1))%sep).
           cancel.
           rewrite (mtree_nonnull n).
           cancel.
  - cancel.
...
Proof.
  unfold insert.
  simp.
  loop_inv (fun p' => mtreep p') (fun p' (_ : unit) => mtreep p').
  - cases (r ==n 0).
    + step.
      step.
      step.
      ...
      cancel.
      equality.
    + apply HtBind with (Q := (fun x => (acc |-> r * exists r1 r2, r |--> [r1; x; r2] * mtree r1 * mtree r2 * [|acc <> 0|])%sep)).
      * pose ((acc |-> r * (exists r1, r |-> r1 * mtree r1) * (exists r2, (r + 1 + 1) |-> r2 * mtree r2) * [|acc <> 0|])%sep) as X.
        apply HtConsequence with (P := (exists x', (r + 1) |-> x' * X)%sep) (Q := fun x' => ((r + 1) |-> x' * X)%sep).
        step.
        rewrite mtree_nonnull by assumption.
        unfold X.
        cancel.
      ...
      * simp.
        cases (x <=? r0).
        -- step.
           apply exis_right with (x := (acc |-> r * (exists y, (r + 1) |-> y) * (exists r2', (r + 1 + 1) |-> r2' * mtree r2') * [| acc <> 0 |])%sep).
           cancel.
           ++ unfold mtreep.
              cancel.
           ++ cancel.
              ...
              cancel.
        -- step.
           apply exis_right with (x := (acc |-> r * (exists y, (r + 1) |-> y) * (exists r1', r |-> r1' * mtree r1') * [| acc <> 0 |])%sep).
           cancel.
           ++ unfold mtreep.
              cancel.
              linear_arithmetic.
